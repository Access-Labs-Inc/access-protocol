...
import { airdropPayer, deployProgram, initializePayer, signAndSendTransactionInstructions, TokenMint, } from "./utils";
import { createCentralState, createStakePool, migrateCentralStateV2, } from "../src/bindings";


import { CentralState, StakePool, Tag, } from "../src/state";
import {
...
beforeAll(async () => {
  ...
  programId = deployProgram(
    payerKeyFile,
    true,
    "days-to-sec-10s no-mint-check no-bond-signer v1-instructions-allowed",
    false
  );
...
test("End to end test", async () => {
  ...
  await airdropPayer(connection, centralStateAuthority.publicKey);
  ...
  let tx = await signAndSendTransactionInstructions(connection, [centralStateAuthority], centralStateAuthority, [
    ix_central_state,
  ]);
  ...
  /**
   * Migrate central state to v2
   */
  const ix = migrateCentralStateV2(
    centralStateAuthority.publicKey, // Central state authority
    programId, // Program ID
  );

  tx = await signAndSendTransactionInstructions(connection, [], centralStateAuthority, [
    ix,
  ]);
