...
async fn program_freeze() {
    ...
    // Renounce the admin functionality
    let renounce_mask = get_freeze_mask(vec![]);
    println!("renounce mask: {:0128b}", renounce_mask);
    let staker = tr.create_user_with_ata().await.unwrap();
    tr.renounce(renounce_mask).await.unwrap();
    ...
    tr.renounce(unfreeze_mask).await.unwrap();
    tr.sleep(1).await.unwrap();
    ...
    tr.renounce(freeze_mask).await.unwrap();
    tr.sleep(1).await.unwrap();
    tr.mint(&staker.pubkey(), 729_999_999_999).await.unwrap_err();
