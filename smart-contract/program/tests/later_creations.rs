...
async fn later_pool_creation() {
    ...
    tr.create_stake_pool(&stake_pool_owner2.pubkey(), 1000).await.unwrap();
    tr.activate_stake_pool(&stake_pool_owner2.pubkey()).await.unwrap();

    // Create stake account
    tr.create_stake_account(&stake_pool_owner2.pubkey(), &staker.pubkey()).await.unwrap();

    // Stake to pool 2
    let token_amount = 10_000;
    tr.stake(&stake_pool_owner2.pubkey(), &staker, token_amount).await.unwrap();
    ...
    // Create stake account
    tr.create_stake_account(&stake_pool_owner2.pubkey(), &staker2.pubkey()).await.unwrap();

    // Try to claim rewards as staker2 from pool2
    tr.stake(&stake_pool_owner2.pubkey(), &staker2, 10_000).await.unwrap();
    tr.claim_staker_rewards(&stake_pool_owner2.pubkey(), &staker2).await.unwrap();
